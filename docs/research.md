# Research

## Prior art

### Curated

- Jupyter
  - JupyterLab
  - https://jupyterbook.org/
  - nbviewer
    - https://nbviewer.org/
    - https://github.com/jupyter/nbviewer
  - nbsanity
    - https://github.com/hamelsmu/nbsanity/
    - https://www.answer.ai/posts/2024-12-13-nbsanity.html
  - FastHTML
    - https://docs.fastht.ml/tutorials/jupyter_and_fasthtml.html
    - https://docs.fastht.ml/api/jupyter.html
- Quarto
- Curvenote
  - https://github.com/curvenote/curvenote
  - https://github.com/rowanc1/phd-thesis
- Grafana Scenes: https://github.com/grafana/scenes
- VitePress
  - https://vitepress.dev/

### Miscellaneous

- https://nbdev.fast.ai/
- https://www.answer.ai/posts/2024-06-23-claudette-src.html
- https://web.archive.org/web/20190216125346/https://streamsets.com/opensource
  - https://www.ibm.com/products/streamsets
  - https://www.softwareag.com/en_corporate/platform/integration-apis/getting-started.html
  - https://github.com/streamsets/datacollector-oss
- https://www.ibm.com/products/cloud-pak-for-data
- https://lambdalabs.com/service/gpu-cloud
- https://fly.io/blog/wrong-about-gpu/
  - https://news.ycombinator.com/item?id=22616857
  - https://news.ycombinator.com/item?id=43053844
- https://github.com/posit-dev/great-tables
- https://tech.ycwu.space/posts/style-table-body/20250124.html
- https://tech.ycwu.space/posts/effective-dataviz-polars-alta-ski-resort/20250210.html
- https://github.com/posit-dev/positron
- Slogan/Phrase: Caddy – The Ultimate Server with Automatic HTTPS 
- https://github.com/awslabs/mountpoint-s3
- https://github.com/leeoniya/uPlot
- https://github.com/saadeghi/daisyui
- https://github.com/auxten/postgresql-parser
- https://github.com/sunng87/pgwire
- https://github.com/wseaton/sqloxide
- https://github.com/Enerview/grafana

## Grafana plugins

- https://grafana.com/api/plugins/grafana-clickhouse-datasource/versions/4.5.1/download?os=linux&arch=amd64
- https://grafana.com/api/plugins/grafana-clock-panel/versions/2.1.8/download
- gridprotectionalliance-osisoftpi-datasource
```json
{
  "name": "aws-datasource-provisioner-app",
  "version": "1.8.1"
}
```
```json
{
    "name": "https://github.com/michaeldmoore/michaeldmoore-multistat-panel/tree/master/dist",
    "version": "1.7.4"
}
```

## Jupyter

### General

- https://github.com/jupyter-book/jupyterlab-myst
- https://session-info2.readthedocs.io/
- https://github.com/jupyter-book/mystmd
  - https://jupyter-book.github.io/myst-theme/
  - https://github.com/simpeg/tle-finitevolume
- https://github.com/IRkernel/IRkernel
- https://jupyter-docker-stacks.readthedocs.io/en/latest/using/selecting.html#jupyter-all-spark-notebook
- https://github.com/jyio/jupyter-marimo-proxy
- https://discourse.jupyter.org/t/jupyter-vs-marimo/28422

### Datalayer

- https://datalayer.tech/
- https://github.com/datalayer/jupyter-server-nbmodel
- https://github.com/jupyterlab/frontends-team-compass
- https://github.com/jupyter-server/jupyter_ydoc
- https://github.com/jupyterlab/jupyter-collaboration
- https://github.com/deshaw/jupyterlab-execute-time
